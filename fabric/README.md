...
3. **Run all cells.** Using the Fabric REST API, the setup notebook:
   ...
   - deploys and **publishes a Fabric data agent** (*Fabric Arch Review - Data Agent*) so the team can ask the review results in plain English — grounded on both the semantic model and the gold lakehouse (and, when the ontology is deployed, the estate ontology as a third source; set `DATA_AGENT_INCLUDE_ONTOLOGY="false"` to keep it to two). Set `DEPLOY_DATA_AGENT="false"` to skip it. Optionally set `ENDORSE_MODEL` (`Promoted`/`Certified`) and `SENSITIVITY_LABEL_ID` (a label GUID) to endorse + label the deployed items.
,
   - deploys a **Fabric IQ estate ontology** (`Fabric_Arch_Review_Estate_Ontology`) — a knowledge graph whose entity types mirror the real gold tables: **Capacity**, **Workspace**, **SemanticModel**, **ModelTable** and **Finding**, connected by **HostedOnCapacity** (Workspace→Capacity), **InWorkspace** (SemanticModel→Workspace) and **BelongsToModel** (ModelTable→SemanticModel) relationships. Set `DEPLOY_ONTOLOGY="false"` to skip it. *(Ontology item names allow only letters, numbers and underscores.)*
   ...
   It is idempotent — re-running it upserts the notebooks, pipeline, model, report, **data agent and ontologyameter (pre-filled with the defaults you set in step 2) so you can adjust scope per run, or *Schedule* it for unattended runs. The collect stage clones this repo, installs `requirements.txt`, authenticates as the **executing identity** via `notebookutils` (no `az login`), gathers metadata into the Lakehouse, then analyze, gold, and report stages run in turn.
