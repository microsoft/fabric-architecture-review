...
,
   - deploys a **Fabric IQ estate ontology** (*Fabric Arch Review - Estate Ontology*) — a knowledge graph whose **EstateNode** entities (capacities, workspaces, models, notebooks, owners…) and **RelatedTo** relationships are bound to the `gold_graph_nodes` / `gold_graph_edges` tables. Set `DEPLOY_ONTOLOGY="false"` to skip it.
