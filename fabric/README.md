...
,
   - deploys a **Fabric IQ estate ontology** (`Fabric_Arch_Review_Estate_Ontology`) — a knowledge graph whose **EstateNode** entities (capacities, workspaces, models, notebooks, owners…) and **RelatedTo** relationships are bound to the `gold_graph_nodes` / `gold_graph_edges` tables. Set `DEPLOY_ONTOLOGY="false"` to skip it. *(Ontology item names allow only letters, numbers and underscores; requires the "Users can create Ontology (preview) items" tenant setting.)*
